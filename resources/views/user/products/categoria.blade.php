...
@section('body')
    ...
    <div class="container">
        <div class="only-mobile">
            @if($tipo==='category')
                <h5 class="titulo segundo" style="color:white !important;">{{$category->nombre}}</h5>
            @else
                <h5 class="titulo segundo" style="color:white !important;">{{$category->categories->nombre.' - '.$category->nombre}}</h5>
            @endif

            <div class="row">
                {!!Form::open(['url'=> Request::path(), 'method'=>'GET'])!!}
                {{-- <div class="col-md-{{($tipo == 'category' ? '3' : '3')}}">
                  <select class="form-control browser-default caract" data-live-search="true" name="filters[]" multiple title="Filtrar por características" id="tags">
                  @foreach($filters as $key=> $filterss)
                    <option value="{{$key}}" @if($filtros) @foreach($filtros as  $filtross) @if($key == $filtross) selected="selected" @endif @endforeach @endif >{{$filterss}}</option>
                  @endforeach

                  </select>
                </div> --}}
                @if($tipo=="subcategory")
                    @foreach($caracteristicCategories as $categoryCaracteristicId=>$categoryCaracteristicName)
                        <div class="col-md-2">
                            <select id="caracteristics" name="caracteristic[{{$categoryCaracteristicName[0]}}][]" class="form-control browser-default {{$categoryCaracteristicName[1]}}" title="{{$categoryCaracteristicName[1]}}" data-live-search="true">
                                @foreach($caracteristicOptions as $option)
                                    @if($option['category']==$categoryCaracteristicName[0])
                                    <option value="{{$option['id_caracteristic']}}">{{$option['caracteristic']}}</option>
                                    @endif
                                @endforeach
                            </select>
                        </div>
                    @endforeach
                @endif
                <div class="col-md-{{($tipo == 'category' ? '2' : '2')}}">
                    {!!Form::select('brands', $brands, null, ['class' => 'form-control browser-default marcas', 'title' => 'Marcas', 'data-live-search' => 'true', 'id' => 'tags', 'data-size' => '5'])!!}
                    {!!Form::hidden('cat', Request::input('cat'))!!}
                    {!!Form::hidden('tipo', $tipo)!!}
                </div>
                @if($tipo == 'category')
                    <div class="col-md-{{($tipo == 'category' ? '2' : '2')}}">
                        {!!Form::select('subcategories', $subcategories, null, ['class' => 'form-control browser-default subcate', 'title' => 'Subcategorias', 'data-live-search' => 'true', 'id' => 'tags', 'data-size' => '5'])!!}
                        {!!Form::hidden('cat', Request::input('cat'))!!}
                        {!!Form::hidden('tipo', $tipo)!!}
                    </div>
                @endif
                ...
                <div class="col-md-1" style="padding-top:14px;">
                    {!!Form::button('<i class="fa fa-search" aria-hidden="true"></i>', array('type' => 'submit', 'class'=>'btn btn-tauret', 'style' => 'padding:0px 13px;margin-top: -8px;height: 29px'))!!}
                </div>
                {!!Form::close()!!}
            </div>
